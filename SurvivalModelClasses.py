...
class Cohort:
    ...
    def get_percentage_survival_5(self):
        """ :returns the percentage of patients survived beyond 5 years"""
        count = 0
        for years in self._survivalTimes:
            if years > 5:
                count += 1

        percentage_survival_5 = count / len(self._survivalTimes)

        return percentage_survival_5

    def get_percentage_survival_5H(self):
        count = []
        for years in self._survivalTimes:
            if years > 5:
                count.append(1)
